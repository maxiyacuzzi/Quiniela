...
create table if not exists resultados (
  ...
  unique (jurisdiccion_slug, turno, fecha)
);

create index if not exists resultados_fecha_idx on resultados (fecha desc);

-- Las jurisdicciones se crean automáticamente la primera vez que corre el scraper
-- (ver lib/ingest.ts), así que no hace falta seedearlas a mano acá.
