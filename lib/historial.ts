...
// Si la fecha ya tiene los 5 turnos con números reales guardados, no vuelve a
// pedirla a la fuente. Si falta alguno (p. ej. se guardó a media tarde, antes
// de Vespertina/Nocturna), vuelve a scrapear para completarla — el upsert no
...
export async function asegurarFecha(fecha: string): Promise<{ disponible: boolean }> {
  ...
  // el número — solo cuenta como "guardado" si tiene al menos un número real.
  const turnosConDatos = new Set(
    (filas ?? []).filter((f) => f.numero !== null).map((f) => f.turno)
  );
  if (TURNOS_ORDEN.every((t) => turnosConDatos.has(t))) return { disponible: true };

  const scrape = await scrapeCabezas(fecha);
  if (!scrape.disponible) return { disponible: turnosConDatos.size > 0 };
